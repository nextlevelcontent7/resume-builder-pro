...
/**
 * Generate a slug that is unique according to the provided async lookup.
 * If a collision occurs, an incrementing suffix will be appended to the slug
 * until the supplied check function returns false.
 * @param {string} str text to slugify
 * @param {Function} exists async function(slug) that resolves to true if slug exists
 * @param {object} opts options passed to slugify
 * @returns {Promise<string>} unique slug
 */
async function slugifyUnique(str, exists, opts = {}) {
  let base = slugify(str, opts);
  let candidate = base;
  let counter = 1;
...
module.exports.slugifyUnique = slugifyUnique;
